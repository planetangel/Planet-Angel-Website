...
else if ($topics_list)
{
	...
	if ($params->get('ul_columns')) {
		$HTML .= '<ul class="mod_dphpbb3_last_topics-wrap">'."\n";
	}
	...
	foreach ($topics_list as $f_id => $f_group)
	{
		...
		foreach ($f_group as $topic)
		{
			...
			if ($row_number == 1)
			{
				$column_number++;
				if ($params->get('ul_columns')) {
					$HTML .= '<li class="mod_dphpbb3_last_topics-column'.$column_number.'">';
				}
				...
				$HTML .= '<ul class="mod_dphpbb3_last_topics-column mod_dphpbb3_last_topics-column'.$column_number.'"'.$style_ul.'>'."\n";
			}
			...
			if ($forum_group)
			{
				if ($forum_group['forum_name'])
				{
					...
					$HTML .= '<li class="mod_dphpbb3_last_topics-forum"'.$style_li.'>'.$forum_name.'</li>'."\n";
				}
				$forum_group = null;
			}
			...
			{
				$posts_replies = '<span class="mod_dphpbb3_last_topics-postr-replies-count">&nbsp;(';
			...
			$HTML .= '	<li class="mod_dphpbb3_last_topics-topic"'.$style_li.'>'.sprintf($TOPIC_OUTPUT_FORMAT, $forum_name, $title_subject, $title, $subject, $username, $date, $topic['topic_replies']+1, $topic['topic_replies']).'</li>'."\n";
		}
	}
	// close last column
	$HTML .= '</ul>';
	...
	if ($params->get('clear_floats')) {
		$HTML .= '<div class="mod_dphpbb3_last_topics-clear" style="clear: left;"></div>'."\n";
	}
}
