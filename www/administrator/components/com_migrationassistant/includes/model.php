...
class JInstallationModel extends JModel
{
	...
	function makeDB($vars = false)
	{
		...
		// we have a valid db connection, bypass this
		/*
		...
		*/
		...
		{
			$DBselect	= false;
			$db = & JFactory::getDBO(); //JInstallationHelper::getDBO($DBtype, $DBhostname, $DBuserName, $DBpassword, null, $DBPrefix, $DBselect);
			...
			if ( ! $db->select($DBname) )
			{
				if (JInstallationHelper::createDatabase($db, $DBname, $DButfSupport))
				{
					...
					$db = NULL;
					$db = & JFactory::getDBO(); //JInstallationHelper::getDBO($DBtype, $DBhostname, $DBuserName, $DBpassword, $DBname, $DBPrefix);
					*/
				} else {
	...
	function dumpLoad() {
		include (MIGBASE . '/includes/bigdump.php');
	}
